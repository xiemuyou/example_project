...
public class TagFlowLayout extends FlowLayout implements TagAdapter.OnDataChangedListener {
    ...
    private void changeAdapter(int lastPosition) {
        removeAllViews();
        TagAdapter adapter = mTagAdapter;
        HashSet preCheckedList = mTagAdapter.getPreCheckedList();
        int size = lastPosition > 0 ? lastPosition : adapter.getCount();
        for (int i = 0; i < size; i++) {
            ...
            if (i == size - 1 && isMaxLineFlag) {
                isMaxLineFlag = false;
                tagViewContainer.addView(adapter.getMoreView(74));
            } else {
    ...
    private boolean isMaxLineFlag;

    @Override
    public void changeLastView(int viewHeight, int changPosition) {
        //部分手机上两行显示无数据
        if (changPosition == 0) {
            return;
        }
        isMaxLineFlag = true;
        changeAdapter(changPosition);
        /*if (tagViewContainer != null && mTagAdapter != null) {
            tagViewContainer.removeViewAt(changPosition);
            tagViewContainer.addView(mTagAdapter.getMoreView(viewHeight), changPosition);
            LogUtil.d("changePosition" + changPosition);
            invalidate();
        }*/
    }
    ...
    public void setMaxSelectCount(int count) {
        if (mSelectedView.size() > count) {
            Log.w(TAG, "you has already select web_more than " + count + " views , so it will be clear .");
            mSelectedView.clear();
        }
        mSelectedMax = count;
    }
