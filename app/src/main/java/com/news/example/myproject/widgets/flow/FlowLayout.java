...
public abstract class FlowLayout extends ViewGroup {
    ...
    public FlowLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mMaxLine = ta.getInt(R.styleable.TagFlowLayout_max_line, -1);
        ta.recycle();
    }
    ...
    /***限制最大行,默认(max_line > 0)限制
     设置最大行数 XML里 :max_line="2" 的参数**/
    private boolean maxLineLimit = true;

    public void setMaxLineLimit() {
        setMaxLineLimit(!maxLineLimit);
    }

    public void setMaxLineLimit(boolean maxLineLimit) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < cCount; i++) {
            ...
            if (maxLineLimit && mMaxLine > 0) {
                addLineHeight = height < childHeight * (mMaxLine - 1);
            }
        ...
        if (!addLineHeight) {
            changeLastView(childHeight, lastPosition);
        }
    ...
    public abstract void changeLastView(int viewHeight, int changPosition);
}
