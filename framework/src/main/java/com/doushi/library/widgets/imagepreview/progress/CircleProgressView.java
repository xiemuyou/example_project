...
public class CircleProgressView extends ProgressBar {
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        switch (mProgressStyle) {
            case ProgressStyle.FILL_IN:
                // 边距 +   直径
                height = getPaddingTop() + getPaddingBottom() + Math.abs(mRadius * 2);
                // 边距 +   直径
                width = getPaddingLeft() + getPaddingRight() + Math.abs(mRadius * 2);
                break;
            case ProgressStyle.FILL_IN_ARC:
                // 边距 +   直径 + 边框
                height = getPaddingTop() + getPaddingBottom() + Math.abs(mRadius * 2) + maxPaintWidth;
                // 边距 +   直径 + 边框
                width = getPaddingLeft() + getPaddingRight() + Math.abs(mRadius * 2) + maxPaintWidth;
                break;
            case ProgressStyle.NORMAL:
                // 边距 +   直径 + 边框
                height = getPaddingTop() + getPaddingBottom() + Math.abs(mRadius * 2) + maxBarPaintWidth;
                // 边距 +   直径 + 边框
                width = getPaddingLeft() + getPaddingRight() + Math.abs(mRadius * 2) + maxBarPaintWidth;
                break;
