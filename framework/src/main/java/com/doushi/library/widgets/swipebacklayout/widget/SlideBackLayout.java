package com.doushi.library.widgets.swipebacklayout.widget;
...
public class SlideBackLayout extends FrameLayout {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (mLock || FApplication.getActivityHelper().getPreActivity() == null) {
    ...
    private void addPreContentView() {
        if (mPreContentView.getParent() != null) {
    ...
    private void removePreContentView() {
        int index = indexOfChild(mPreContentView);
        if (index != -1) {
            removeViewAt(index);
        }
    }
...
//            Log.e("====","onViewPositionChanged height:" + getHeight());
