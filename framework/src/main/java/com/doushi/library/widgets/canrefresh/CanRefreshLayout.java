...
public class CanRefreshLayout extends FrameLayout {
    ...
    private void smoothMove(boolean isHeader, boolean isMove, int moveScrollY, int moveY) {
        ...
        if (listener != null) {
            listener.onScroll(moveY);
        }
        if (!isRelease && null != mHeaderView) {
            ((ClassicRefreshView) mHeaderView).setCurrentFrame(moveY / 5);
        }
        if (isHeader) {
    ...
    public void refreshComplete() {
        ...
        postDelayed(new Runnable() {
            @Override
            public void run() {
//                ValueAnimator animator = ValueAnimator.ofInt((int)lastY,0);
