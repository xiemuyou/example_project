package com.library.widgets.scrollview;
...
public class ReboundHorizontalScrollView extends HorizontalScrollView {
    ...
    private OnScrollViewTouch onScrollViewTouch;
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                //判断是否可以右拉和左拉
                if (onScrollViewTouch != null) {
                    onScrollViewTouch.onTouch();
                }
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (isTriggerRight) {
                    //绝对值
                    if (offset < 0 && Math.abs((float) offset / getWidth()) > 0.3) {
                        if (null != mOnReboundListener) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                //是否应该移动布局
                //可以左拉， 并且手指向左移动
                boolean shouldMove = (canPullLeft && deltaX < 0)
                        //可以右拉， 并且手指向右移动
                        || (canPullRight && deltaX > 0)
...
//        return contentView.getWidth() <= getWidth() + getScrollX();
    ...
    public void setOnScrollViewTouch(OnScrollViewTouch onScrollViewTouch) {
        this.onScrollViewTouch = onScrollViewTouch;
    }
    ...
    private interface ScrollViewTouchListener {
    }
}
